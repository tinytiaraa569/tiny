...
function DashboardMessages() {
    ...
    useEffect(() => {
        ...
        return () => {
            socketId.off("getMessage"); // Clean up event listener on unmount or effect cleanup
        };
...
const SellerInbox = ({ setOpen, newMessage, setNewMessage, sendMessageHandler, messages, activeStatus, sellerId, userData, handleImageUpload }) => {
    return (
        <div className='w-full min-h-full flex flex-col overflow-hidden justify-between mt-4 '>
            ...
            <div className="px-3 h-[65vh] overflow-y-scroll">
                {
                     messages.map((item) => (
                        <div className={`${item.sender === sellerId ? 'justify-end' : 'justify-start'} w-full flex  my-3`}>
                            {
